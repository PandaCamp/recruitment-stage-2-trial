  ...
  return (
    <div className={style['inputItemContainer']}>
      ...
      <div className={style['inputBox']}>
        {props.fieldShow && (
          <div className={style['fieldLeft']} style={{color: props.disabled ? '#8c8c8c' : ''}}>
            <label>{props.disabled ? '+86' : '大陆手机号'}</label>
            <span onClick={props.handleDialog} className={style['toggleState']}>
              ^
            </span>
        ...
        <div className={style['inputRight']}>
          ...
          />
          <div className={style['close']} onClick={clearInput} style={{display: flag ? 'block' : 'none'}}>
            {clearIcon}
          </div>
        </div>
      </div>
